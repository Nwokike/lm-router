...
def ServerScreen():
    ...
    if running:
        ...
        source_counts = counts.get("sources") or {}
        summary = ""
        if model_counts:
            summary = (
                f"{model_counts.get('active', 0)} active"
                + (
                    f" · {model_counts.get('degraded', 0)} degraded"
        ...
        if source_counts:
            sources = (
                f"{source_counts.get('responsive', 0)} of {source_counts.get('total', 0)} "
                "sources responding"
            )
        conn_card = ft.Container(
            ...
            content=ft.Column(
                ...
                controls=[
                    ...
                    *([_kv_row("Sources", sources, None, is_dark)] if sources else []),
                    ft.Text(
