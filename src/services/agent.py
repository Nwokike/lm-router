...
class AgentService:
    ...
    def ensure_kani(self, model: str, system_prompt: str) -> Kani | None:
        ...
        self._base = base
        LOG.info("kani ready model=%s base=%s", model, base)
        return self._kani
        ...
        async def _run() -> None:
            try:
                async for manager in kani.full_round_stream(prompt):
                    # v1 has no tools, so every manager is the assistant stream.
                    # S5 (tools) must skip FUNCTION-role managers for on_done.
