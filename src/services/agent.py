...
def _default_engine_factory(
    ...
    engine = ReasoningEngine(
        ...
        tokenizer=get_encoding_for_model(model)[0],
        tap=tap,
    )
    LOG.info(
...
class AgentService:
    ...
    def start_turn(
        ...
        if estimated > budget_for_prompt and kani.chat_history:
            LOG.warning(
                "prompt exceeds budget (%d > %d), truncating oldest history",
                estimated,
                budget_for_prompt,
            )
            truncated, dropped = truncate_history_to_budget(
                kani.chat_history,
                budget_tokens=budget_for_prompt - count_tokens(prompt),
                model=self._model,
            )
            kani.chat_history.clear()
            kani.chat_history.extend(truncated)
            LOG.info("truncated %d messages from chat history", dropped)
