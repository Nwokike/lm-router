...
def build_server_params(server: MCPServerConfig, is_mobile: bool = False) -> object:
    if server.transport == "stdio":
        ...
        # A stdio server inherits the parent process environment; `headers` is
        # for HTTP auth and must NOT be reinterpreted as env vars. Passing
        # them through also meant an auth header like "Authorization" leaked
        # into the child's environment.
        return StdioServerParameters(command=server.command, args=list(server.args), env=None)
    ...
    async def _connect(self) -> None:
        ...
        for server in servers:
            try:
                ...
                LOG.warning("mcp %s skipped: %s", server.name, exc.message)
                failed.append(f"{server.name}: {exc.message}")
                continue
            ...
            try:
                ...
                failed.append(f"{server.name}: timed out")
                continue
            except Exception as exc:
                LOG.warning("mcp %s failed: %s", server.name, exc)
                failed.append(f"{server.name}: {str(exc)[:120]}")
                continue
