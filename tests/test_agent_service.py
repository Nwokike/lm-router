...
def test_engine_always_receives_a_tokenizer() -> None:
    """kani must never fall back to its unbounded tiktoken download.

    Its lazy `tokenizer` property downloads BPE ranks with no timeout when
    given None — 40-60s on a cold machine, and a first-turn hang in CI where
    no cache exists. The engine must be handed one on construction.
    """
    from services.agent import _default_engine_factory as factory
    from services.tokenizer import tokenizer_or_heuristic

    tok = tokenizer_or_heuristic("some-unknown-model-zz")
    assert tok is not None, "tokenizer_or_heuristic returned None"
    assert len(tok.encode("hello world")) > 0

    built = factory(AppSettings(), "some-unknown-model-zz")
    assert getattr(built, "_tokenizer", None) is not None, (
        "kani would lazily download BPE ranks with no timeout"
    )
...
def test_streamed_turn_delivers_deltas_and_usage(monkeypatch) -> None:
    ...
    try:
        ...
        assert done.wait(60), f"turn stalled (network fetch?); errors={seen['errors']}"
        assert seen["errors"] == []
