...
def test_share_controls_exist_on_the_controller_surface(stub_page):
    """The Server tab's Share card needs these three controller methods."""
    from state.controller_ctx import ControllerMethods

    for name in ("start_share", "stop_share", "regenerate_share_key"):
        assert hasattr(ControllerMethods(), name), f"missing {name}"


def test_share_settings_default_to_off_and_opt_in(stub_page):
    from core.settings import AppSettings

    settings = AppSettings()
    assert settings.share_enabled is False
    assert settings.require_share_key is False
    assert settings.share_key == ""


def test_server_screen_renders_a_share_card(stub_page):
    """With a running gateway the Server tab must offer sharing."""
    from core.state import state
    from screens.server_screen import ServerScreen
